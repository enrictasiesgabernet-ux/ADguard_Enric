...
**Pràctica 0378 · Filtratge DNS amb Docker · Tallafocs ASIC**  
**Autor:** Enric (enrictasiesgabernet-ux)  
**Institut:** IES El Calamot · Gavà
...
- Debian instal·lat a VirtualBox
- Docker i Docker Compose instal·lats
- Git configurat

### 1. Instal·lar Docker a Debian
...
# Afegir usuari al grup docker (evitar sudo)
...
```bash
git clone https://github.com/enrictasiesgabernet-ux/ADguard_Enric.git
cd ADguard_Enric
```

### 3. Alliberar el port 53 (important a Debian!)

Debian utilitza `systemd-resolved` que ocupa el port 53. Cal alliberar-lo:
...
# Eliminar l'enllaç simbòlic de resolv.conf
sudo rm /etc/resolv.conf

# Crear un resolv.conf manual temporal
echo "nameserver 8.8.8.8" | sudo tee /etc/resolv.conf

# Verificar que el port 53 és lliure
...
### 5. Configuració inicial (Setup Wizard)
...
> 💡 Per trobar la IP de la MV: `ip addr show` o `hostname -I`
...
- **Portabilitat:** El mateix `docker-compose.yml` funciona en qualsevol sistema amb Docker
- **Persistència:** Els volums `adguard_work` i `adguard_conf` garanteixen que la configuració sobreviu reinicis del contenidor
- **Aïllament:** El servei corre en la seva pròpia xarxa bridge `adguard_net`
- **Facilitat de manteniment:** `docker compose up/down` per gestionar el cicle de vida
...
├── adguard_conf/           # Configuració persistent (generat per Docker)
│   └── AdGuardHome.yaml
├── adguard_work/           # Dades de treball (generat per Docker)
│   └── data/
│       └── querylog.json
└── screenshots/            # Captures de pantalla
    ├── 01-dashboard.png
    ├── 02-blocklist.png
    ├── 03-query-blocked.png
    └── 04-dns-config.png
```
...
Projecte educatiu per a la pràctica 0378 del Cicle Formatiu ASIC — IES El Calamot, Gavà.
